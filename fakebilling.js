      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              if (isSelectedProduct) {
                // ✅ Already removed from UI, exit early
                return;
              }
              ...
              // ✅ Ensure UI updates again after Supabase response (fallback)
              setTimeout(() => {
                setOrders((prevOrders) =>
                  prevOrders.map((order) =>
              ...
              {appointments.map((appt, index) => (
                <View key={index} style={styles.appointmentCard}>
                  ...
                  <View style={styles.buttonContainer}>
                    ...
                    >
                      <FontAwesome name="undo" size={15} color="white" />
                    </TouchableOpacity>
                  </View>
                </View>
              ))}
...
{paymentMethod && selectedOrder?.order_no === order.order_no && (
  <View>
    <TextInput
    ...
    <TouchableOpacity
      ...
      onPress={() => handlePaymentConfirmation(order)}
    >
      <Text style={styles.confirmButtonText}>Payment</Text>
                    ...
                    <Text
                      style={{ fontSize: 14, color: "gray", marginTop: 10 }}
                    >
                      No orders found.
                    </Text>
                  )}
